...
export default async function AdminServicesPage() {
  ...
  const [servicesRes, categoriesRes, providersRes] = await Promise.all([
    supabase.from("services").select("id, name, category_id, provider_id, provider_service_id, price, provider_price, min_quantity, max_quantity, average_time, type, description, is_active, is_featured, profit_margin, created_at").order("created_at", { ascending: false }).limit(1000),
